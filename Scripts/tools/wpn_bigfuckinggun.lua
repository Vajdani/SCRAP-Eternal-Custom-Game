...
function BFG.loadAnimations( self )
...
function BFG.client_onUpdate( self, dt )
...
function BFG.client_onEquip( self, animate )
...
function BFG.client_onUnequip( self, animate )
...
function BFG.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function BFG.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function BFG.onShoot( self, dir )
...
function BFG.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function BFG.calculateFpMuzzlePos( self )
...
function BFG.cl_onPrimaryUse( self, state )
	...
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), se_ammo_argent, 30 ) then
			local firstPerson = self.tool:isInFirstPersonView()
...
function BFG.client_onEquippedUpdate( self, primaryState, secondaryState )
