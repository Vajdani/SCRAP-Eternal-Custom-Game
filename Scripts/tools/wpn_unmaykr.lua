...
function Unmaykr.client_onCreate( self )
...
function Unmaykr.client_onRefresh( self )
	self:loadAnimations()
end

function Unmaykr.loadAnimations( self )
...
--SE
function Unmaykr:client_onFixedUpdate( dt )
    if not self.tool:isLocal() then return end

	local ownerChar = self.tool:getOwner().character
	if ownerChar == nil then return end
	if ownerChar:isSwimming() or ownerChar:isDiving() or not self.tool:isEquipped() then return end

	if self.isFiring then
		self.fireCounter = self.fireCounter + dt
		if (self.fireCounter/0.2) > 1 then
			if not sm.game.getEnableAmmoConsumption() or sm.container.canSpend( sm.localPlayer.getInventory(), se_ammo_argent, 1 ) then
				self:cl_shootProjectile( proj_unmaykr, Damage )
			else
...
function Unmaykr.cl_shootProjectile( self, projectileType, projectileDamage )
    ...
    if owner then
        sm.projectile.projectileAttack(  projectileType, projectileDamage, firePos, sm.vec3.rotate( dir, math.rad(-10), sm.camera.getUp()) * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf )
        sm.projectile.projectileAttack(  projectileType, projectileDamage, firePos, sm.vec3.rotate( dir, math.rad(-5), sm.camera.getUp())  * fireMode.fireVelocity, owner, fakePosition, fakePositionSelf )
...
function Unmaykr.client_onUpdate( self, dt )
...
function Unmaykr.client_onEquip( self, animate )
	...
	if self.tool:isLocal() then
		-- Sets Unmaykr renderable, change this to change the mesh
		self.tool:setFpRenderables( currentRenderablesFp )
		swapFpAnimation( self.fpAnimations, "unequip", "equip", 0.2 )
	end
end

function Unmaykr.client_onUnequip( self, animate )
...
function Unmaykr.sv_n_onShoot( self, dir )
	self.network:sendToClients( "cl_n_onShoot", dir )
end

function Unmaykr.cl_n_onShoot( self, dir )
	if not self.tool:isLocal() and self.tool:isEquipped() then
		self:onShoot( dir )
	end
end

function Unmaykr.onShoot( self, dir )
...
function Unmaykr.calculateFirePosition( self )
...
function Unmaykr.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )
	local right = sm.localPlayer.getRight()
...
function Unmaykr.calculateFpMuzzlePos( self )
...
function Unmaykr.cl_onPrimaryUse( self, state )
	if self.tool:getOwner().character == nil then
		return
	end
...
function Unmaykr.client_onEquippedUpdate( self, primaryState, secondaryState )
	--SE
	self.isFiring = primaryState == sm.tool.interactState.start or primaryState == sm.tool.interactState.hold
    --SE
