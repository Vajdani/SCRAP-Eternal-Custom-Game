...
function MechanicCharacter.client_onCreate( self )
	...
	self.canBloodPunch = false
	self.canNormalPunch = false
end
...
function MechanicCharacter.client_onGraphicsLoaded( self )
	...
	self.animations.parry = {
		info = self.character:getAnimationInfo( "parry" ),
		time = 0,
		weight = 0
	}
	self.animations.normalPunch = {
...
function MechanicCharacter:sv_sendBPEvent( player )
	sm.event.sendToPlayer( player, "sv_bloodPunch" )
end

function MechanicCharacter:sv_sendNPEvent( player )
	sm.event.sendToPlayer( player, "sv_normalPunchAttack" )
end
	...
	if self.isLocal then
		for name, animation in pairs( self.FPanimations ) do
			if animation.info then
				...
				if self.canBloodPunch and animation.info.name == "punch" and animation.time < animation.info.duration and animation.time >= animation.info.duration / 10 then
					self.network:sendToServer("sv_sendBPEvent", sm.localPlayer.getPlayer())
					self.canBloodPunch = false
				elseif animation.info.name == "punch" and animation.time >= animation.info.duration then
					self.canBloodPunch = true
				end

				if self.canNormalPunch and animation.info.name == "normalPunch" and animation.time < animation.info.duration and animation.time >= animation.info.duration / 10 then
					self.network:sendToServer("sv_sendNPEvent", sm.localPlayer.getPlayer())
					self.canNormalPunch = false
				elseif animation.info.name == "normalPunch" and animation.time >= animation.info.duration then
...
function MechanicCharacter.cl_handleEvent( self, event )
	...
	if self.currentAnimation == "" then
		...
		elseif event == "normalPunch" then
			...
			if self.isLocal then
				self.currentFPAnimation = "normalPunch"
				self.FPanimations.normalPunch.time = 0
			end
		elseif event == "resetVars" then
			self.canBloodPunch = false
			self.canNormalPunch = false
